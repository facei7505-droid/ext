...
export class Orchestrator {
  ...
  private async handleClearAll(): Promise<RpaResult> {
    ...
    await this.deps.sendToTab({ type: 'rpa:reloadTab' }).catch(() => {});
    await this.deps.sendToTab({
      type: 'rpa:speak',
      text: 'Внесенные данные полностью очищены.',
    }).catch(() => {});
    await this.setState('IDLE');
  ...
  private async handleShowFields(): Promise<RpaResult> {
    ...
    await this.deps.sendToTab({
      type: 'rpa:speak',
      text: 'На этой странице я могу автоматически заполнить: жалобы, анамнез, объективный статус, давление, пульс и диагнозы пациента.',
    }).catch(() => {});
    return { ok: true, data: { fields: ['жалобы', 'анамнез', 'давление', 'пульс', 'температура', 'диагнозы'] } };
  }
  ...
  private async handleHelp(): Promise<RpaResult> {
    ...
    await this.deps.sendToTab({
      type: 'rpa:speak',
      text: 'Я ваш голосовой ассистент КМИС. Вы можете легко продиктовать мне весь прием пациента, или управлять системой с помощью команд типа «сохрани форму», «перейди на дневник», «очисти всё».',
    }).catch(() => {});
  ...
  private async handleRepeat(): Promise<RpaResult> {
    ...
    await this.deps.sendToTab({
      type: 'rpa:speak',
      text: 'Повторяю. Я жду вашей диктовки или голосовой команды.',
    }).catch(() => {});
  ...
  private async handleSave(): Promise<RpaResult> {
    ...
    const r = await this.deps.sendToTab({ type: 'rpa:clickSubmit' });
    if (r.ok) {
      await this.deps.sendToTab({
        type: 'rpa:speak',
      ...
      await this.deps.sendToTab({
        type: 'rpa:speak',
        text: 'Не нашел кнопку сохранения на данной странице.',
      }).catch(() => {});
    }
    await this.setState('IDLE');
    return r;
  ...
  private async handleNavigate(target: string): Promise<RpaResult> {
    await this.setState('FILLING_DOM', `Переход: ${target}`);
    
    let url = '';
    const t = target.toLowerCase();
    
    if (t.includes('эпикриз')) {
      url = '/#/epicrisis-damumed';
    } else if (t.includes('дневник') || t.includes('запис')) {
      url = '/#/diary-damumed';
    } else if (t.includes('осмотр') || t.includes('прием')) {
      url = '/#/intake-damumed';
    } else if (t.includes('истори') || t.includes('рекорд')) {
      url = '/#/intake-damumed';
    } else if (t.includes('назначен') || t.includes('расписан') || t.includes('процедур')) {
      url = '/#/assignments-damumed';
    } else if (t.includes('диагноз')) {
      url = '/#/diagnoses-damumed';
    }
    
    if (url) {
      await this.deps.sendToTab({
        type: 'rpa:speak',
        text: `Осуществляю переход в нужный раздел`,
      }).catch(() => {});
      
      const r = await this.deps.sendToTab({
        type: 'rpa:navigateTab',
        url: url
      });
      await this.setState('IDLE');
      return r;
      ...
      await this.deps.sendToTab({
        type: 'rpa:speak',
        text: `Не удалось найти раздел ${target}`,
      }).catch(() => {});
      await this.setState('IDLE');
      return { ok: false, error: 'Unknown target', code: 'BAD_ARG' };
    }
  }
