...
async function handleCommand(msg: BackgroundToContentMsg): Promise<RpaResult> {
  switch (msg.type) {
    ...
    case 'rpa:clickSubmit': {
      const submitBtn = document.querySelector<HTMLElement>('button[type="submit"], .btn-primary, .btn-save');
      if (submitBtn) {
        submitBtn.click();
        return { ok: true };
      }
      return { ok: false, error: 'Кнопка сохранения не найдена', code: 'NOT_FOUND' };
    }
